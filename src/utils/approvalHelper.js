...
// approvalHelper.js — fix hasApprovalRelevantChanges

// approvalHelper.js — fix the FK field name derivation

export async function hasApprovalRelevantChanges(
  referencePage,
  existingRecord,
  newBody,
) {
  ...
  const fields = await prisma.approvalRuleField.findMany({
    where: { moduleId: module.id, active: true },
    select: { name: true, parentRelation: true, aggregation: true },
  });

  for (const field of fields) {
    // ── CASE 1: Flat field ────────────────────────────────────────────────────
    if (!field.parentRelation) {
      const oldVal = existingRecord[field.name];
      const newVal = newBody[field.name];
      if (
        oldVal !== undefined &&
        newVal !== undefined &&
        ...
        console.log(
          `🔄 Flat field changed: ${field.name} (${oldVal} → ${newVal})`,
        );
        return true;
      }
    }

    // ── CASE 2: Array relation with aggregation (e.g. poItems.qty SUM) ────────
    else if (field.aggregation) {
      const relationKey =
        field.parentRelation.charAt(0).toLowerCase() +
        field.parentRelation.slice(1);

      const oldItems =
        existingRecord[relationKey] ||
        existingRecord[field.parentRelation] ||
        [];
      const newItems =
        newBody[relationKey] || newBody[field.parentRelation] || [];

      const oldAgg = computeAggregation(
        oldItems,
        field.name,
        field.aggregation,
      );
      const newAgg = computeAggregation(
        newItems,
        field.name,
        field.aggregation,
      );

      if (Math.abs(oldAgg - newAgg) > 0.001) {
        console.log(
          `🔄 Array field "${field.name}" aggregation changed: ${oldAgg} → ${newAgg}`,
        );
        return true;
      }
    }

    // ── CASE 3: Single relation (e.g. Supplier.name) ──────────────────────────
    else if (field.parentRelation) {
      // ✅ FIX: derive FK correctly
      // "Supplier" → look for "supplierId" on the record (lowercase first letter + Id)
      const relationLower =
        field.parentRelation.charAt(0).toLowerCase() +
        field.parentRelation.slice(1);

      // Try common FK patterns: supplierId, branchId, etc.
      const fkCandidates = [
        `${relationLower}Id`, // supplierId
        `${field.parentRelation}Id`, // SupplierId (unlikely but safe)
        `${relationLower}_id`, // supplier_id (snake_case)
      ];

      let changed = false;
      for (const fkField of fkCandidates) {
        const oldFk = existingRecord[fkField];
        const newFk = newBody[fkField];

        if (oldFk !== undefined && newFk !== undefined) {
          // ✅ Compare as integers to avoid "5" !== 5 false positives
          if (parseInt(oldFk) !== parseInt(newFk)) {
            console.log(
              `🔄 Relation FK changed: ${fkField} (${oldFk} → ${newFk})`,
            );
            changed = true;
          }
          break; // Found the FK field, stop checking candidates
        }
      }
  ...
  console.log(`✅ No approval-relevant fields changed — silent edit`);
  return false;
}

// ── Helper: compute aggregation value from an array of items ─────────────────
function computeAggregation(items, fieldName, aggregation) {
