...
import {  roleGroup } from "../../../Utils/DropdownData";
...
export default function RoleMaster() {
  ...
  const [roleGroupVal, setRoleGroupVal] = useState("");
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetRolesQuery({ params, searchParams: searchValue });
  const {
    data: singleData,
  ...
  const syncFormWithDb = useCallback(
    (data) => {
      if (id) {
        ...
        setPages((prev) =>
          prev.map((page) => {
            const currentRoleOnPage = data?.RoleOnPage.find(
              (item) => parseInt(item?.page?.id) === parseInt(page.id),
            );
            let newPage = { ...page };
      ...
      setRoleGroupVal(data?.type ? data?.type : "")
  ...
  const data = {
    ...
    type: roleGroupVal,
    companyId: secureLocalStorage.getItem(
      sessionStorage.getItem("sessionId") + "tempCompanyId",
    )
  ...
  return (
    ...
    >
      <div className="flex flex-col frame w-full h-full">
        <FormHeader
          childRecordValidationActions={["delete"]}
          onNew={onNew}
          onClose={() => {
            setForm(false);
            setSearchValue("");
          ...
          <div className="col-span-3 grid md:grid-cols-3 border">
            <div className="mr-1 md:ml-2 col-span-3 flex flex-col">
              <fieldset className="frame my-1">
                ...
                <div className="grid grid-cols-3 my-2 justify-items-center">
                  <TextInput
                    name="Role Name"
                    type="text"
                    value={name}
              ...
              <fieldset
                className="frame md:grid md:justify-items-stretch overflow-auto h-[420px]"
                style={{ width: "100%" }}
              >
                ...
                <table className="table-auto text-center">
                  <thead className="border-2 table-header">
                    <tr>
                      {tableHeadersPages.map((head, index) => (
                        <th
                          key={index}
                  ...
                  <tbody
                    className={` ${readOnly ? "border-2 pointer-events-none" : "border-2"}`}
                  >
                    {pages.map((dataObj, index) => (
                      <tr key={index} className="border-2 table-row">
                        <td key={index} className="table-data text-start">
                          {dataObj.name}
                        </td>
                        <td
