...
const SUPPLIERS   = ["SP APPARELS", "KPR TRADERS", "MUTHU TEXTILES", "RAJA FABRICS", "SRI TEXTILES", "ANAND MILLS"];
const BRANCHES    = ["MUTHU PRINTERS", "COIMBATORE", "TIRUPPUR", "ERODE"];
const PO_TYPES    = ["GENERAL", "ORDER"];
const INWARD_TYPES= ["General Purchase Inward", "Order Purchase Inward", "Direct Inward"];
const STATUSES    = ["Pending", "Partially Received", "Fully Received", "Cancelled", "Partially Cancelled", "Closed (Inward + Cancelled)"];
const ITEMS       = ["BROWN BOARD", "WHITE BOARD", "KRAFT PAPER", "CORRUGATED SHEET", "BUBBLE WRAP", "STRETCH FILM", "TISSUE PAPER"];
...
export const dummyData = Array.from({ length: 120 }, (_, idx) => {
  const id         = 100 + idx;
  const poNo       = idx + 1;
  ...
  const poItems    = Array.from({ length: itemCount }, (_, i) => makeItem(id * 10 + i, id));
  ...
  const inwardDocs = hasInward ? [{
    ...
    receiptType,
    dcNo:        `DC-${Math.floor(Math.random() * 9000 + 1000)}`,
    invNo:       receiptType === "Against Invoice" ? `INV/${Math.floor(Math.random()*9000+1000)}` : null,
    vehicleNo:   `TN39-${Math.floor(Math.random()*9000+1000)}`,
    store:       "WAREHOUSE",
    supplier,
    netBillValue:  receiptType === "Against Invoice" ? rand(500, 50000) : null,
    discountType:  receiptType === "Against Invoice" ? (Math.random() > 0.5 ? "Percentage" : "Flat") : null,
  ...
  const billItems = hasBill
    ...
    : [];
  const billedQty = receiptType === "Against Invoice"
    ? inwardQty
  ...
  else if (inwardQty > 0 && cancelQty > 0) status = "Partially Received & Cancelled";
