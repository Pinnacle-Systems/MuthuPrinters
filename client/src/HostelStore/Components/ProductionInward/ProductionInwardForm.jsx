...
import InwardDetails, {
  DEFAULT_ROW_COUNT,
  makeEmptyRow,
} from "./InwardDetails.jsx";
...
const padItems = (itemsArray = []) => {
  ...
  if (currentLength < DEFAULT_ROW_COUNT) {
    const padding = Array.from(
      { length: DEFAULT_ROW_COUNT - currentLength },
      () => ({
        ...EMPTY_ROW,
      }),
    );
...
const ProductionInwardForm = ({
  ...
  const { data: outwardList } = useGetProductionOutwardQuery({
  ...
  useEffect(() => {
    ...
    isProductionOutwardDtlsLoading,
    isProductionOutwardDtlsFetching,
    syncFormWithDbItems,
    productionOutwardDtlsData,
  ]);
  ...
  const handleOutwardChange = (item) => {
    ...
    if (item.productionOutwardDetails?.length) {
      ...
      const padded = [
        ...rows,
        ...Array.from(
          { length: Math.max(0, DEFAULT_ROW_COUNT - rows.length) },
          makeEmptyRow,
        ),
      ];
  ...
  const enrichedItems = useMemo(() => {
    ...
    const { items, ...totals } = calculateTaxWithHSNBreakupAndInsertIntoPoItems(
                ...
                {receiptType === "AGAINST_INVOICE" && (
                  <div>
                    ...
                    <div className="flex justify-between text-[12px] font-medium">
                      ...
                      <span>Rs.{parseFloat(totals?.net || 0).toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </div>
