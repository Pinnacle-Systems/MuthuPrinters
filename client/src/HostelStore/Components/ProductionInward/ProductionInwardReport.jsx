...
const ProductionInwardReport = ({
    ...
    const searchFields = { searchDocNo, searchDocDate, searchSupplier, searchJobCard };

    useEffect(() => {
        setCurrentPageNumber(1);
    }, [searchDocNo, searchDocDate, searchSupplier, searchJobCard]);
                        ...
                        <table className="">
                            <thead className="bg-gray-200 text-gray-800">
                                <tr>
                                    ...
                                    <th className="w-14 px-3 font-medium text-[13px] text-gray-900 text-center" rowSpan={2}>
                                        <div>Actions</div>
                                    </th>
                                </tr>
                                <tr>
                                    ...
                                    <th className="px-1 font-medium text-[13px] text-gray-900 text-center w-32">
                                        {/* Outward No — no search needed, leave blank */}
                                    </th>
                            ...
                            {isLoadingIndicator ? (
                                ...
                                <tbody className="border-2">
                                    {(allData?.data ? allData?.data : []).map((dataObj, index) => (
                                        <tr
                                            onKeyDown={(e) => { if (e.key === "Enter") onClick(dataObj.id); }}
                                            tabIndex={0}
                                            key={dataObj.id}
                                        ...
                                        >
                                            ...
                                            <td className="py-1.5 text-left">
                                                {dataObj?.ProductionOutward?.docId}
                                            </td>
                                            <td className="py-1.5 text-left">
                                                ...
                                                <td className="px-2 py-1">
                                                    <div className="flex items-center justify-center">
                                                        <div className="flex items-center gap-1 pl-2">
                                                            ...
                                                            {onDelete && (
                                                                <Tooltip
                                                                    title={dataObj.childRecord > 0 ? "Cannot Delete. Child Record Exists" : "Delete"}
                                                                    arrow
                                                                >
                                                                    <button
                                                                        className={`flex items-center gap-1 px-1 rounded transition ${dataObj.childRecord > 0
                                                                            ? "bg-red-50 text-red-500 opacity-40 cursor-not-allowed"
                                                                            : "bg-red-50 text-red-800 hover:bg-red-100"
                                                                            }`}
                                                                        onClick={(e) => { e.stopPropagation(); onDelete(dataObj.id); }}
